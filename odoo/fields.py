...
class Field(MetaField("DummyField", (object,), {})):
    ...
    def _get_attrs(self, model, name):
        ...
        modules = set()
        attrs = {}
        if self.args.get("automatic") and resolve_mro(
            model, name, self._can_setup_from
        ):
        ...
        if attrs.get("company_dependent"):
            ...
            attrs["depends_context"] = attrs.get("depends_context", ()) + (
                "force_company",
            )
    ...
    def _setup_attrs(self, model, name):
        ...
        for key, msg in DEPRECATED_ATTRS:
            if key in attrs:
                _logger.warning(
                    "Field %s: parameter %r is not longer supported; %s", self, key, msg
                )
        ...
        if self.index:
            try:
                with model._cr.savepoint(flush=False):
                    sql.create_index(
                        model._cr, indexname, model._table, ['"%s"' % self.name]
                    )
    ...
    def __get__(self, record, owner):
        ...
        if (
            self.compute
            and (record.id in env.all.tocompute.get(self, ()))
    ...
    def __set__(self, records, value):
        ...
        if new_ids:
            ...
            with records.env.protecting(
                records._field_computed.get(self, [self]), records
            ):
...
class Monetary(Field):
    ...
    def _setup_currency_field(self, model):
        ...
        assert (
            self.currency_field in model._fields
    ...
    def convert_to_cache(self, value, record, validate=True):
        ...
        if value and validate:
            ...
            # BEWARE: do not prefetch other fields, because 'value' may be in
            # cache, and would be overridden by the value read from database!
            currency = record.sudo().with_context(prefetch_fields=False)[
                self.currency_field
            ]
            if len(currency) > 1:
                raise ValueError(
...
class _String(Field):
    ...
    _slots = {"translate": False, "prefetch": None}  # whether the field is translated
...
class Image(Binary):
    ...
    _slots = {"max_width": 0, "max_height": 0, "verify_resolution": True}
        ...
        for field in reversed(resolve_mro(model, name, self._can_setup_from)):
            ...
            if "selection_add" in field.args:
                ...
                assert values is not None, (
                    ...
                    % (self, selection_add, self.selection)
                )
...
class _RelationalMulti(_Relational):
    ...
    def convert_to_record(self, value, record):
        ...
        corecords = record.pool[self.comodel_name]._browse(
            record.env, value, prefetch_ids
        )
        if "active" in corecords and self.context.get(
                ...
                if to_inverse:
                    for record, inverse_ids in to_inverse.items():
                        ...
                        lines = lines.filtered(
                            lambda line: int(line[inverse]) != record.id
                        )
...
class Many2many(_RelationalMulti):
    ...
    def update_db(self, model, columns):
        ...
        if not sql.table_exists(cr, self.relation):
            ...
            sql.fix_foreign_key(
                cr, self.relation, self.column2, comodel._table, "id", self.ondelete
            )
    ...
    def write_real(self, records_commands_list, create=False):
        ...
        comodel = (
            records_commands_list[0][0]
            .env[self.comodel_name]
