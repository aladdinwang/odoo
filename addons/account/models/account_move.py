...
class AccountMove(models.Model):
    ...
    def _get_default_journal(self):
        ...
        if self._context.get("default_journal_id"):
            ...
            if move_type != "entry" and journal.type != journal_type:
                raise UserError(
                    _(
                        "Cannot create an invoice of type %s with a journal having %s as type."
                    )
    ...
    amount_by_group = fields.Binary(
        string="Tax amount by group", compute="_compute_invoice_taxes_by_group"
    )
    ...
    reversed_entry_id = fields.Many2one(
        "account.move", string="Reversal of", readonly=True, copy=False
    )
    ...
    invoice_partner_icon = fields.Char(
        compute="_compute_invoice_partner_display_info", store=False, compute_sudo=True
    )
    ...
    invoice_sequence_number_next_prefix = fields.Char(
        string="Next Number Prefix", compute="_compute_invoice_sequence_number_next"
    )
    ...
    def _onchange_currency(self):
        ...
        if self.is_invoice(include_receipts=True):
            ...
            has_foreign_currency = (
                self.currency_id and self.currency_id != company_currency
            )
    ...
    def _onchange_type(self):
        ...
        if self.is_sale_document(include_receipts=True):
            if (
                ...
                self.narration = (
                    self.company_id.invoice_terms or self.env.company.invoice_terms
                )
        ...
        def _apply_cash_rounding(
            ...
            if self.invoice_cash_rounding_id.strategy == "biggest_tax":
                ...
                if diff_balance > 0.0:
                    ...
                    account_id = (
                        self.invoice_cash_rounding_id._get_profit_account_id().id
                    )
                rounding_line_vals.update(
                    {
    ...
    def _recompute_payment_terms_lines(self):
        ...
        def _compute_diff_payment_terms_lines(
            ...
            for date_maturity, balance, amount_currency in to_compute:
                if (
                    self.journal_id.company_id.currency_id.is_zero(balance)
                    and len(to_compute) > 1
                ):
                    continue
    ...
    def _compute_type_name(self):
        type_name_mapping = {
            k: v for k, v in self._fields["type"]._description_selection(self.env)
        }
    ...
    def _compute_invoice_sequence_number_next(self):
        ...
        for key, group in groupby(
            moves, key=lambda move: (move.journal_id, move._get_sequence())
        ):
            ...
            for move in group:
                ...
                move.invoice_sequence_number_next = (
    ...
    def _compute_invoice_taxes_by_group(self):
        ...
        for move in self:
            ...
            for line in tax_lines:
                res.setdefault(
                    line.tax_line_id.tax_group_id, {"base": 0.0, "amount": 0.0}
                )
    ...
    def _check_fiscalyear_lock_date(self):
        for move in self.filtered(lambda move: move.state == "posted"):
            lock_date = max(
                move.company_id.period_lock_date or date.min,
    ...
    def _move_autocomplete_invoice_lines_values(self):
        ...
        for line in self.line_ids:
            ...
            if (
                not line._cache.get("account_id")
    ...
    def _move_autocomplete_invoice_lines_write(self, vals):
        ...
        for invoice in self:
            invoice_new = invoice.with_context(
                default_type=invoice.type, default_journal_id=invoice.journal_id.id
            ).new(origin=invoice)
    ...
    def write(self, vals):
        ...
        if (
            "state" in vals and vals.get("state") == "posted"
        ) and self.restrict_mode_hash_table:
            for move in self.filtered(
                lambda m: not (m.secure_sequence_number or m.inalterable_hash)
            ):
    ...
    def _track_subtype(self, init_values):
        ...
        if (
            "invoice_payment_state" in init_values
            and self.invoice_payment_state == "paid"
        ):
    ...
    def _get_invoice_reference_euro_partner(self):
        ...
        partner_ref_nr = (
            re.sub("\D", "", partner_ref or "")[-21:] or str(self.partner_id.id)[-21:]
        )
    ...
    def _get_sequence(self):
        ...
        if (
            self.type
            in ("entry", "out_invoice", "in_invoice", "out_receipt", "in_receipt")
    ...
    def _get_reconciled_payments(self):
        ...
        reconciled_amls = pay_term_line_ids.mapped(
    ...
    def _reverse_moves(self, default_values_list=None, cancel=False):
        ...
        for move, default_values in zip(self, default_values_list):
            default_values.update(
                {"type": reverse_type_map[move.type], "reversed_entry_id": move.id}
            )
            ...
            for move, reverse_move in zip(self, reverse_moves):
                ...
                for account in accounts:
                    (move.line_ids + reverse_move.line_ids).filtered(
                        lambda line: line.account_id == account and line.balance
                    ).reconcile()
    ...
    def message_new(self, msg_dict, custom_values=None):
        ...
        if (custom_values or {}).get("type", "entry") not in (
            "out_invoice",
            "in_invoice",
        ):
        ...
        followers = [
            partner
            for partner in self._mail_find_partner_from_emails(cc_mail_addresses)
            if partner
        ]
    ...
    def post(self):
        ...
        for move in self:
            if move.auto_post and move.date > fields.Date.today():
                raise UserError(
                    _(
                        "This move is configured to be auto-posted on {}".format(
    ...
    def action_open_matching_suspense_moves(self):
        ...
        ids = (
            self.env["account.move.line"].search(domain).mapped("statement_line_id").ids
        )
        action_context = {
    ...
    def action_switch_invoice_into_refund_credit_note(self):
        ...
        for move in self:
            ...
            move.write(
                {"invoice_line_ids": [(5, 0, 0)], "invoice_partner_bank_id": False}
            )
    ...
    def action_view_reverse_entry(self):
        ...
        if len(reverse_entries) == 1:
            ...
            action.update(
                {"view_mode": "tree", "domain": [("id", "in", reverse_entries.ids)]}
            )
        return action
...
class AccountMoveLine(models.Model):
    ...
    ref = fields.Char(
        related="move_id.ref", store=True, copy=False, index=True, readonly=False
    )
    ...
    def _get_fields_onchange_subtotal_model(
        ...
        if currency and currency != company.currency_id:
            ...
            balance = currency._convert(
                price_subtotal, company.currency_id, company, date
            )
            return {
    ...
    def _get_fields_onchange_balance_model(
        ...
        if taxes and any(tax.price_include for tax in taxes):
            ...
            # 10            |               | 5%                | 10                | 10
            taxes_res = taxes._origin.compute_all(
                balance, currency=currency, handle_price_include=False
            )
    ...
    @api.onchange(
        "amount_currency", "currency_id", "debit", "credit", "tax_ids", "account_id"
    )
        ...
        for line in self:
            ...
            if line.tax_ids and line.move_id.fiscal_position_id:
                ...
                line.price_unit = line._get_fields_onchange_balance(
                    balance=balance
                ).get("price_unit", line.price_unit)

            # Convert the unit price to the invoice's currency.
            company = line.move_id.company_id
            line.price_unit = company.currency_id._convert(
                line.price_unit, line.move_id.currency_id, company, line.move_id.date
            )
    ...
    def _onchange_uom_id(self):
        ...
        if taxes and self.move_id.fiscal_position_id:
            ...
            balance = accounting_vals["debit"] - accounting_vals["credit"]
            price_unit = self._get_fields_onchange_balance(balance=balance).get(
    ...
    def _onchange_amount_currency(self):
        for line in self:
            ...
            if not line.move_id.is_invoice(include_receipts=True):
                ...
                continue
            line.update(line._get_fields_onchange_balance(balance=line.amount_currency))
            line.update(line._get_price_total_and_subtotal())
    ...
    def _amount_residual(self):
        ...
        for line in self:
            ...
            for partial_line in line.matched_debit_ids + line.matched_credit_ids:
                ...
                sign_partial_line = (
                    sign if partial_line.credit_move_id == line else (-1 * sign)
                )
            ...
            line.amount_residual = (
                line.move_id.company_id.currency_id.round(amount * sign)
                if line.move_id.company_id
                else amount * sign
            )
            line.amount_residual_currency = (
                line.currency_id
                and line.currency_id.round(amount_residual_currency * sign)
                or 0.0
            )
        ...
        for record in self:
            ...
            for tag in record.tag_ids:
                tag_amount = (
                    ...
                    * (record.move_id.is_inbound() and -1 or 1)
                    * record.balance
                )
    ...
    def _check_constrains_account_id(self):
        for line in self:
            ...
            control_type_failed = (
                ...
                and account.user_type_id not in journal.type_control_ids
            )
            control_account_failed = (
    ...
    def _affect_tax_report(self):
        ...
        return (
            self.tax_ids
            or self.tax_line_id
            or self.tag_ids.filtered(lambda x: x.applicability == "taxes")
        )
    ...
    def create(self, vals_list):
        ...
        for vals in vals_list:
            ...
            if move.is_invoice(include_receipts=True):
                ...
                taxes = self.resolve_2many_commands(
                    "tax_ids", vals.get("tax_ids", []), fields=["id"]
                )
            ...
            if vals.get("tax_repartition_line_id"):
                ...
                tax_ids = [
                    v["id"]
                    for v in self.resolve_2many_commands(
    ...
    def write(self, vals):
        ...
        def field_will_change(line, field_name):
            ...
            if field.type in ("one2many", "many2many"):
                ...
                after_write_ids = set(
                    r["id"]
                    for r in line.resolve_2many_commands(
            ...
            if "statement_line_id" in vals and line.payment_id:
                ...
                if all(
                    ...
                    for line in line.payment_id.move_line_ids.filtered(
                        ...
                        and r.account_id.internal_type == "liquidity"
                    )
                ):
        ...
        for line in self:
            ...
            if any(field in cleaned_vals for field in ACCOUNTING_FIELDS):
                ...
                to_write = line._get_fields_onchange_balance(
                    balance=balance, price_subtotal=price_subtotal
                )
                to_write.update(
    ...
    def check_full_reconcile(self):
        ...
        if (
            ...
            or (
                cash_basis_partial
                and all([p >= 1.0 for p in amls._get_matched_percentage().values()])
            )
        ) and (
            currency
            and float_is_zero(
                total_amount_currency, precision_rounding=currency.rounding
            )
            or multiple_currency
            and float_compare(
                total_debit, total_credit, precision_rounding=digits_rounding_precision
            )
            == 0
        ):
    ...
    def _reconcile_lines(self, debit_moves, credit_moves, field):
        ...
        while debit_moves and credit_moves:
            ...
            if amount_reconcile == -credit_move[field]:
                ...
                credit_moves[
                    0
                ].amount_residual_currency += temp_amount_residual_currency
        ...
        for after_rec_dict in cash_basis_subjected:
            ...
            if cash_basis and not (
                new_rec.debit_move_id.move_id
                == new_rec.credit_move_id.move_id.reversed_entry_id
                or new_rec.credit_move_id.move_id
                == new_rec.debit_move_id.move_id.reversed_entry_id
            ):
    ...
    def reconcile(self, writeoff_acc_id=False, writeoff_journal_id=False):
        ...
        not_paid_invoices = self.mapped("move_id").filtered(
            lambda m: m.is_invoice(include_receipts=True)
            and m.invoice_payment_state not in ("paid", "in_payment")
        )
        ...
        for line in self:
            if not matched_percentage_per_move.get(line.move_id.id, False):
                    ...
                    for line in lines_to_consider:
                        if all_same_currency:
                            ...
                            for partial_line in (
                                ...
                                if (
                                    partial_line.currency_id
                                    and partial_line.currency_id.id == all_same_currency
                                ):
                                    total_reconciled_currency += (
                ...
                if not all_same_currency:
                    ...
                    matched_percentage_per_move[
                        line.move_id.id
                    ] = line.move_id._get_cash_basis_matched_percentage()
                else:
                    ...
                    if total_amount_currency == 0.0:
                        ...
                        currency = (
                            lines_to_consider[0].currency_id
                            or lines_to_consider[0].company_id.currency_id
                        )
                        matched_percentage_per_move[line.move_id.id] = currency.round(
                            total_reconciled_currency
                        ) / currency.round(total_amount_currency)
    ...
    def action_accrual_entry(self):
        [action] = self.env.ref(
            "account.account_accrual_accounting_wizard_action"
        ).read()
...
class AccountPartialReconcile(models.Model):
    ...
    full_reconcile_id = fields.Many2one(
        "account.full.reconcile", string="Full Reconcile", copy=False
    )
            ...
            aml_model.with_context(check_move_validity=False).create(
                {
                    ...
                    "debit": aml.amount_residual > 0 and aml.amount_residual or 0.0,
                    "credit": aml.amount_residual < 0 and -aml.amount_residual or 0.0,
                    "account_id": aml.amount_residual > 0
    ...
    def _set_tax_cash_basis_entry_date(self, move_date, newly_created_move):
        if (
            move_date > (self.company_id.period_lock_date or date.min)
        ...
        for move in {self.debit_move_id.move_id, self.credit_move_id.move_id}:
            ...
            for line in move.line_ids:
                if not line.tax_exigible:
                    ...
                    if (
                        line.tax_line_id
                        and line.tax_line_id.tax_exigibility == "on_payment"
                    ):
                    ...
                    taxes_payment_exigible = line.tax_ids.flatten_taxes_hierarchy().filtered(
    ...
    def _create_tax_basis_move(self):
        ...
        move_vals = {
            ...
            if self.credit_move_id.payment_id
            else self.debit_move_id.move_id.name,
        }
    ...
    def unlink(self):
        ...
        for rec in self:
            if rec.exists() and rec.exchange_move_id:
                ...
                if to_reverse.date > (
                    to_reverse.company_id.period_lock_date or date.min
                ):
