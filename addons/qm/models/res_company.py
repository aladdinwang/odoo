...
class ResCompany(models.Model):
    ...
    def _create_dropship_return_sequence(self):
        vals = []
        for company in self:
            vals.append(
                {
                    "name": "Dropship Return (%s)" % company.name,
                    "code": "stock.dropshipping.return",
                    "company_id": company.id,
                    "prefix": "DSR/",
                    "padding": 5,
                }
    ...
    def create_missing_dropship_sequence(self):
        ...
        company_has_dropship_return_seq = (
            self.env["ir.sequence"]
            .search(["code", "=", "stock.dropshipping.return"])
            .mapped("company_id")
        )
        company_todo_sequence = company_ids - company_has_dropship_return_seq
        company_to_sequence._create_dropship_return_sequence()

    def _create_per_company_sequences(self):
        super(ResCompany, self)._create_per_company_sequences()
        self._create_dropship_sequence()
        self._create_dropship_return_sequence()
    ...
    # 代发货（退货）
    def _create_dropship_return_picking_type(self):
        vals = []
        for company in self:
            sequence = self.env["ir.sequence"].search(
                [
    ...
    def create_missing_dropship_picking_type(self):
        ...
        company_has_dropship_return_picking_type = (
            self.env["stock.picking.type"]
            .search([("sequence_code", "=", "DSR")])
            .mapped("company_id")
        )
        company_todo_picking_type = (
            company_ids - company_has_dropship_return_picking_type
        )
        company_todo_picking_type._create_dropship_return_picking_type()
    ...
    def _create_per_company_picking_types(self):
        ...
        self._create_dropship_return_picking_type()
    ...
    def _create_per_company_rules(self):
        ...
        self._create_purchase_request_rule()
