...
class SaleOrder(models.Model):
    ...
    def _search_invoice_ids(self, operator, value):
        ...
        return [
            "&",
            (
                "order_line.invoice_lines.move_id.type",
                "in",
    ...
    def _compute_remaining_validity_days(self):
        for record in self:
            if record.validity_date:
                record.remaining_validity_days = (
                    record.validity_date - fields.Date.context_today(record)
                ).days + 1
            else:
        ...
        if self._context.get("sale_show_partner_name"):
            if operator == "ilike" and not (name or "").strip():
                ...
                return models.lazy_name_get(
                    self.browse(order_ids).with_user(name_get_uid)
                )
                ...
                ref_invoice_vals.update(
                    {
                        ...
                        "invoice_payment_ref": len(payment_refs) == 1
...
class SaleOrderLine(models.Model):
    ...
    product_custom_attribute_value_ids = fields.One2many(
        "product.attribute.custom.value", "sale_order_line_id", string="Custom Values"
    )
    ...
    order_partner_id = fields.Many2one(
        related="order_id.partner_id", store=True, string="Customer", readonly=False
    )
